...
class TestFileLoading(unittest.TestCase):
    def test_load_yaml_config(self):
        ...
        assert (
            experiment_config.meta_job_args.base_train_fname
            == cmd_args_proxy.base_train_fname
        )
        assert (
            experiment_config.meta_job_args.base_train_config
            == cmd_args_proxy.base_train_config
        )
        assert (
            experiment_config.meta_job_args.experiment_dir
    ...
    def test_load_result_logs(self):
        ...
        meta, hyper = load_result_logs(
            "tests/unit/fixtures/experiment_1", meta_log_fname, hyper_log_fname
        )

    def test_load_meta_log(self):
        """Assert correct loading of meta log files."""
        meta = load_meta_log(
            os.path.join("tests/unit/fixtures/experiment_1", meta_log_fname)
        )

    def test_load_hyper_log(self):
        """Assert correct loading of hyper log files."""
        hyper = load_hyper_log(
            os.path.join("tests/unit/fixtures/experiment_1", hyper_log_fname)
        )

    def test_reload_hyper_log(self):
        """Assert correct reloading of hyper log files for continuation."""
        hyper = HyperoptLogger(
            hyperlog_fname=os.path.join(
                "tests/unit/fixtures/experiment_1", hyper_log_fname
            ),
            eval_metrics="integral",
            reload_log=True,
        )
        assert hyper.iter_id == 4
        assert hyper.best_per_metric == {
            "integral": {
                "run_id": 4,
                "score": 4.274364709854126,
                "params": {"noise_mean": 0.01, "x_0": 10.0},
            }
        }

    def test_combine_experiments(self):
        """Test loading of multiple experiment logs."""
        meta, hyper = combine_experiments(
            ["tests/unit/fixtures/experiment_1", "tests/unit/fixtures/experiment_2"]
        )
