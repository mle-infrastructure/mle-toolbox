...
experiment_dir = "examples/experiments/random"
...
def test_run_random_async() -> None:
    """ Test job launch wrapper - Run PDE experiment on local machine. """
...
def test_api_random_sync() -> None:
    """ Execute `mle run pde_search_random.yaml` and check running pipeline. """
    os.chdir('./examples')
    exp_dir = "experiments/random/api_test"
    # Remove experiment dir at start of test
    if os.path.exists(exp_dir) and os.path.isdir(exp_dir):
        shutil.rmtree(exp_dir)

    # Execute the mle api command
    bashCommand = ("mle run numpy_pde/pde_search_random.yaml -nw -np -resource local"
